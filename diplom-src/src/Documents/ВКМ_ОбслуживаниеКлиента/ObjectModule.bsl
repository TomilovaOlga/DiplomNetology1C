...
//ВКМ_Диплом Автор="ТомиловаО.Н." Комментарий="Расчет Зарплаты.", Дата = 17.05.2024{
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		РезультатЗапроса = Запрос.Выполнить();
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				ДвижениеСотрудник.ЧасовКОплате = ВыборкаДетальныеЗаписи.ФактическиПотраченоЧасов;
				ДвижениеСотрудник.ВидДвижения = ВидДвиженияНакопления.Приход;
				ДвижениеСотрудник.СуммаКОплате = ДвижениеСотрудник.ЧасовКОплате * ВыборкаДетальныеЗаписи.ВКМ_СтоимостьЧасаРаботыСпециалиста * ПроцентОтРабот / 100;  
			КонецЦикла;
		
	Иначе
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = СтрШаблон("Не возможно проведение документа %1. По сотруднику %2 не установлен процент оплаты от работ.", Ссылка, Специалист);
			Сообщение.Сообщить();
	КонецЕсли;
// НетологияДиплом}
